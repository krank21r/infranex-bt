...
class TestGPUInstallRoutes:
    def test_install_cuda_endpoint(self):
        response = client.post("/gpu-engine/test-server/install-cuda")
        assert response.status_code == 200

    def test_install_docker_endpoint(self):
        response = client.post("/gpu-engine/test-server/install-docker")
        assert response.status_code == 200

    def test_install_dependencies_endpoint(self):
        response = client.post("/gpu-engine/test-server/install-dependencies",
                               json={"requirements_txt_url": "test"})
        assert response.status_code == 200

    def test_download_model_endpoint(self):
        response = client.post("/gpu-engine/test-server/download-model",
                               json={"model_url": "hf://test/repo", "target_path": "/tmp/model"})
        assert response.status_code == 200

    def test_write_config_endpoint(self):
        response = client.post("/gpu-engine/test-server/write-config",
                               json={"config": {"hotkey": "0x123"}})
        assert response.status_code == 200

    def test_setup_complete_endpoint(self):
        response = client.post("/gpu-engine/test-server/setup-complete")
        assert response.status_code == 200

    def test_setup_status_endpoint(self):
        response = client.get("/gpu-engine/test-server/setup-status")
        assert response.status_code == 200